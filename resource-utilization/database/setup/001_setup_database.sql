...
-- Creates schemas, roles, and schema-level grants only.
...
-- (table-level SELECT on bronze/seeds added via post-hook)
...
-- (table-level SELECT on bronze/seeds/silver/gold via post-hook)
...
-- (table-level SELECT granted by dbt post-hook on mart models)
GRANT USAGE ON SCHEMA mart_gold TO business_user;

-- partner_dw_engineer: data engineers from other internal teams who consume
-- specific silver tables to build their own pipelines on top of this warehouse.
-- Gets USAGE on seeds and staging_silver — but table-level SELECT is granted
-- via post-hook on specific models only, not blanket access to all silver.
-- No access to bronze, staging_silver_ds, or gold.
